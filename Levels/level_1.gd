...
func _ready() -> void:
	...
	$DoorOpenedArea2D.set_collision_mask_value(12, true)
	
	# connect the body entered event
	if not $DoorOpenedArea2D.body_entered.is_connected(_on_close_door_opened_area_2d_body_entered):
		$DoorOpenedArea2D.body_entered.connect(_on_close_door_opened_area_2d_body_entered)
...
func _on_close_door_opened_area_2d_body_entered(body: Node2D) -> void:
	if !LevelProgress.is_completed(level.name, level.interactions.have_opened_the_door.key):
		...
		if player:
			...
			LevelProgress.mark_as_completed(level.name, level.interactions.have_opened_the_door.key)
